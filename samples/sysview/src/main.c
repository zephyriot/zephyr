...
{
#if defined(CONFIG_KERNEL_EVENT_LOGGER_TASK)
	uint32_t thread_id;

	thread_id = event_data[1];

	switch ((enum sys_k_event_logger_task_event)event_data[2]) {
	case KERNEL_LOG_TASK_EVENT_READYQ:
		SEGGER_SYSVIEW_OnTaskStartReady(thread_id);
		break;
	case KERNEL_LOG_TASK_EVENT_PEND:
		SEGGER_SYSVIEW_OnTaskStopReady(thread_id, 3<<3);
		break;
	case KERNEL_LOG_TASK_EVENT_EXIT:
		SEGGER_SYSVIEW_OnTaskTerminate(thread_id);
		break;
	}

	/* FIXME: Maybe we need a KERNEL_LOG_TASK_EVENT_CREATE? */
#endif
}
...
static uint32_t zephyr_to_sysview(int event_type)
{
	static const uint32_t lut[] = {
		...
		[KERNEL_EVENT_LOGGER_TASK_EVENT_ID] =
...
static void sysview_setup(void)
{
	...
	evs = SYSVIEW_EVTMASK_PRINT_FORMATTED |
		...
		ENABLE_SYSVIEW_EVENT(KERNEL_EVENT_LOGGER_TASK);
...
static void sysview_thread(void)
{
	...
	for (;;) {
		...
		case KERNEL_EVENT_LOGGER_SLEEP_EVENT_ID:
			...
			break;
		case KERNEL_EVENT_LOGGER_TASK_EVENT_ID:
